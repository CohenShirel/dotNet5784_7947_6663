...
public static class Initialization
{
    ...
    private static void creatAssignmentss()
    {
        ...
        foreach (var _name in AssignmentssNames)
        {
            ...
            DateOnly _date = DateOnly.FromDateTime(DateTime.Now);// help date
            DateOnly _dateStart = _date.AddDays(s_rand.Next(30, 61));//random date for "datestart" in range of month=תאריך מתוכנן להתחלה
            DateOnly _deadLine = _dateStart.AddYears(s_rand.Next(1, 4));// "_dateBegin" late in month from datestart=תאריך מתוכנן לסיום
            DateOnly _dateBegin = _dateStart.AddDays(s_rand.Next(30, 61));//when we are start=תאריך התחלה בפועל
            DateOnly _dateFinish = _deadLine.AddDays(s_rand.Next(30,61));//when we are finish=,תאריך סיום בפועל
    ...
    private static void creatLink()
    {
        ...
        s_dalLink!.Create(new Link(0, 4, 5));
        s_dalLink!.Create(new Link(0, 4, 6));
        s_dalLink!.Create(new Link(0, 5, 7));
        s_dalLink!.Create(new Link(0, 5, 8));
        s_dalLink!.Create(new Link(0, 6,7));
        s_dalLink!.Create(new Link(0, 6,8));
        s_dalLink!.Create(new Link(0, 7, 9));
        s_dalLink!.Create(new Link(0, 7, 10));
        s_dalLink!.Create(new Link(0, 7, 11));
        s_dalLink!.Create(new Link(0, 7, 12));
        s_dalLink!.Create(new Link(0, 7,13));
        s_dalLink!.Create(new Link(0, 7, 14));
        s_dalLink!.Create(new Link(0, 8,9));
        s_dalLink!.Create(new Link(0, 8,10));
        s_dalLink!.Create(new Link(0, 8,11));
        s_dalLink!.Create(new Link(0, 8,12));
        s_dalLink!.Create(new Link(0, 8,13));
        s_dalLink!.Create(new Link(0, 8,14));
        s_dalLink!.Create(new Link(0, 9,15));
        s_dalLink!.Create(new Link(0, 9,17));
        s_dalLink!.Create(new Link(0, 10,15));
        s_dalLink!.Create(new Link(0, 10,17));
        s_dalLink!.Create(new Link(0, 11,15));
        s_dalLink!.Create(new Link(0, 11, 17));
        s_dalLink!.Create(new Link(0, 12, 15));
        s_dalLink!.Create(new Link(0, 12,17)); 
        s_dalLink!.Create(new Link(0, 13,15));
        s_dalLink!.Create(new Link(0, 13,17));
        s_dalLink!.Create(new Link(0, 14,15));
        s_dalLink!.Create(new Link(0, 14,17));
        s_dalLink!.Create(new Link(0, 15, 16));
        s_dalLink!.Create(new Link(0, 17,18));
        s_dalLink!.Create(new Link(0, 18,19));
        s_dalLink!.Create(new Link(0, 19,20));
        s_dalLink!.Create(new Link(0, 20,21));
        s_dalLink!.Create(new Link(0, 21, 22));
        s_dalLink!.Create(new Link(0, 22, 23));
        s_dalLink!.Create(new Link(0, 16, 21));
    }
}
